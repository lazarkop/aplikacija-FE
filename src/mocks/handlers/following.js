...
export const getUserFollowingMock = rest.get(
  `${BASE_URL}/user/following`,
  (req, res, ctx) => {
    const result = {
      message: 'User following',
      following: [existingUserTwo, existingUserThree],
    };
    return res(ctx.json(result));
  }
);

export const emptyUserFollowingMock = rest.get(
  `${BASE_URL}/user/following`,
  (req, res, ctx) => {
...
export const getUserFollowersMock = rest.get(
  ...
  (req, res, ctx) => {
    const result = {
      ...
      followers: [existingUserTwo, existingUserThree],
    };
    return res(ctx.json(result));
  }
);
