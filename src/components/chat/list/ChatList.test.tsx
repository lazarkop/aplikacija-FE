...
import { addToChatList } from "../../../../redux-toolkit/reducers/chat/chat.reducer";
...
describe("ChatList", () => {
  ...
  it("should display user image and username", () => {
    ...
    expect(headerImage.childNodes.item(0)).toHaveAttribute(
      "src",
  ...
  it("should chat chat url when an item is clicked", async () => {
    ...
    act(() => {
      store.dispatch(
        addToChatList({
          isLoading: false,
          chatList: [messageData, messageDataTwo],
        })
      );
    });
    const conversationItems = await screen.findAllByTestId("conversation-item");
  ...
  it("should display new search item at the top of list", async () => {
    ...
    act(() => {
      store.dispatch(
        addToChatList({
          isLoading: false,
          chatList: [messageDataTwo, messageData],
        })
      );
    });
    const conversationItems = await screen.findAllByTestId("conversation-item");
