...
describe("Post", () => {
  beforeEach(() => {
    ...
    window.localStorage.setItem(
      "selectedPostId",
      JSON.stringify(`${postMockData._id}`)
    );
  });
  ...
  it("should have username", async () => {
    ...
    expect(usernameElement.childNodes.item(0).textContent).toEqual(
  ...
  it("should display post time and privacy", async () => {
    const { baseElement } = render(
      <Post post={postMockData} showIcons={false} />
    );
  ...
  it("should display post with colored background", async () => {
    ...
    expect(userPostElement).toBeInTheDocument();
    expect(userPostElement).toHaveStyle({
