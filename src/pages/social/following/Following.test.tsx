...
describe("Following", () => {
  ...
  it("should have card element buttons", async () => {
    render(<Following />);
    const cardElementButtons = await screen.findAllByTestId(
  ...
  it("should handle follow button", async () => {
    ...
    expect(FollowersUtils.followUser).toHaveBeenCalledWith(
      existingUserTwo,
      expect.any(Function)
    );
  });
  ...
  it("should handle unfollow button", async () => {
    ...
    userEvent.click(cardElementButtons[0].children[0]);
    expect(cardElementButtons[0].children[0].textContent).toEqual("Unfollow");
    expect(socketService.socket.emit).toHaveBeenCalledWith(
      "unfollow user",
      existingUserTwo
    );
    ...
    expect(FollowersUtils.socketIORemoveFollowing).toHaveBeenCalledWith(
      [],
      expect.any(Function)
    );
  });
