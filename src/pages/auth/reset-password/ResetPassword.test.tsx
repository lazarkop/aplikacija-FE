...
describe("ResetPassword", () => {
  ...
  it("should be enabled with input", () => {
    ...
    const newPasswordElement = screen.getByLabelText("New Password");
    const confirmPasswordElement = screen.getByLabelText("Confirm Password");
    userEvent.type(newPasswordElement, "qwerty1");
    userEvent.type(confirmPasswordElement, "qwerty1");
  ...
  describe("Error", () => {
    it("should display error alert and border", async () => {
      ...
      const confirmPasswordElement = screen.getByLabelText("Confirm Password");
      userEvent.type(newPasswordElement, "qwerty1");
      userEvent.type(confirmPasswordElement, "qwerty");
      userEvent.click(buttonElement);

      const alert = await screen.findByRole("alert");
      expect(alert).toBeInTheDocument();
      await waitFor(() =>
        expect(newPasswordElement).toHaveStyle({ border: "1px solid #fa9b8a" })
      );
      await waitFor(() =>
        expect(newPasswordElement).toHaveStyle({ border: "1px solid #fa9b8a" })
      );
